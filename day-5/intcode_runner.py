...
def get_val(program, input, mode):
    if mode == 0:
...
def intcode_runner(program):
    ...
    while True:
        ...
        if opcode == 1:
            ...
            print('output: {}'.format(get_val(program, program[pointer + 1], mode_1)))
            pointer += 2
        elif opcode == 5:
            ...
            program[program[pointer + 3]] = int(get_val(program, program[pointer + 1], mode_1) < get_val(program, program[pointer + 2], mode_2))
            pointer += 4
        elif opcode == 8:
            program[program[pointer + 3]] = int(get_val(program, program[pointer + 1], mode_1) == get_val(program, program[pointer + 2], mode_2))
            pointer += 4
